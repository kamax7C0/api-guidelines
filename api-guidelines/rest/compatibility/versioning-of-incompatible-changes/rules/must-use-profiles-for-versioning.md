...
A profile is identified by a URI such as `https://api.companyname.de/products/profiles/product+v1` see [MUST use absolute URIs for profiles](../../../compatibility/versioning-of-incompatible-changes/rules/must-use-absolute-profile-uris.md).
...
By adding the media type parameter profile (i.e., `application/hal+json;profile="https://api.companyname.de/products/profiles/product+v1"`), we specify the needed additional semantics relevant for the product representation, so that API consumers know about the different properties of the `product`.
If a second version of the `product` representation is added, the new version of the spec (`https://api.companyname.de/products/profiles/product+v2`) can be distinguished from the first version.
...
```http request
POST https://api.companyname.de/products/ HTTP/1.1
Accept: application/hal+json; profile="https://api.companyname.de/products/profiles/product+v1"
Content-Type: application/json; profile="https://api.companyname.de/products/profiles/product+v1"

{
... //request body conforming to profile https://api.companyname.de/products/profiles/product+v1
}
```
